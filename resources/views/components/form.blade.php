<form {{$attributes->merge(['class'])}}  class=" w-[1100px]  py-4 " action="{{ $action }}" method="{{ $method }}">
        @csrf
        {{$slot}}
        <div {{$attributes->merge(['class'])}} class="bg-white shadow-md rounded-md flex flex-col ">
